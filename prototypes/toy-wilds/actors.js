...
const crystalGeometry = facetedCrystal();
...
function instantiateBaked(parts, name) {
  const group = new THREE.Group();
  group.name = name;
  for (const part of parts) {
    const item = mesh(group, part.geometry, part.material);
...
function buildExplorerParts() {
  ...
  // Broad, low cream hat; the head remains anonymous, with no facial features.
  mesh(body, primitive.cylinder, palette.cream, [0, 1.007, 0.015], [0.28, 0.043, 0.235]);
  const crown = new THREE.CylinderGeometry(0.125, 0.198, 0.157, 8);
  mesh(body, crown, palette.cream, [0, 1.105, 0.004]);
  mesh(body, primitive.cylinder, palette.ochre, [0, 1.045, 0.004], [0.199, 0.042, 0.199]);

  // Backpack is on -Z, preserving +Z as the traveler's forward direction.
  mesh(body, beveledPlate(0.285, 0.30, 0.125, 0.025), palette.orange, [0, 0.606, -0.175]);
  mesh(body, beveledPlate(0.305, 0.082, 0.155, 0.014), palette.brown, [0, 0.731, -0.184]);
  mesh(body, beveledPlate(0.13, 0.10, 0.045, 0.010), palette.ochre, [0, 0.537, -0.265]);
  for (const side of [-1, 1]) {
    mesh(body, primitive.cube, palette.orange, [side * 0.108, 0.626, 0.113], [0.035, 0.205, 0.034]);
  }
  mesh(body, primitive.cylinder, palette.orange, [0, 0.816, -0.189], [0.071, 0.35, 0.071], [0, 0, Math.PI / 2]);
  for (const side of [-1, 1]) {
    mesh(body, primitive.cylinder, palette.brown, [side * 0.12, 0.816, -0.189], [0.075, 0.022, 0.075], [0, 0, Math.PI / 2]);
  }
...
export function createExplorer() {
  ...
  const body = instantiateBaked(explorerBodyParts, 'Traveler body');
  group.add(body);
  const legs = [];
  const arms = [];
  for (const side of [-1, 1]) {
    const leg = instantiateBaked(explorerLegParts, side < 0 ? 'Left leg' : 'Right leg');
    leg.position.set(side * 0.092, 0.396, 0);
    group.add(leg);
    legs.push(leg);
    const arm = instantiateBaked(explorerArmParts, side < 0 ? 'Left arm' : 'Right arm');
