...
pub fn set_tool_enabled_internal(
    ...
    if enabled {
        ...
        let ids: Vec<String> = store.get_all_targets().map_err(AppError::db)?.into_iter()
            .filter(|target| target.tool == key).map(|target| target.skill_id).collect();
        crate::core::foundation_write::deploy(store, &ids, &[key.to_string()], false)?;
        set_disabled_tools(store, &disabled)
    }
}
