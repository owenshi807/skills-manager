...
pub async fn sync_skill_to_tool(
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        let outcome = crate::core::foundation_write::deploy(&store, &[skill_id.clone()], &[tool.clone()], true);
...
pub async fn unsync_skill_from_tool(
    ...
    let store = store.inner().clone();
    let result = tauri::async_runtime::spawn_blocking(move || {
        let outcome = crate::core::foundation_write::deploy(&store, &[skill_id.clone()], &[tool.clone()], false);
