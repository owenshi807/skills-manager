...
use app_lib::core::{
    ...
    merge, repo_lock::RepoLock, scenario_service, skill_metadata,
...
#[command(about = "Skill Card Manager Foundation CLI", version = app_lib::core::cli_bridge::CONTRACT_VERSION)]
struct Cli {
...
enum Commands {
    ...
    Tools(ToolsArgs),
    Skills(SkillsArgs),
    #[command(alias = "scenarios", hide = true)]
    Presets(PresetArgs),
    Git(GitArgs),
}
...
enum SkillsCommand {
    ...
    /// Show actual per-Agent deployment state.
    Status {
        reference: String,
    },
    #[command(hide = true)]
...
/// Reject hidden product state before opening or initializing the store.
fn validate_foundation_surface(command: &Commands) -> anyhow::Result<()> {
    let hidden = match command {
        Commands::Presets(_) => true,
        Commands::Skills(args) => match &args.command {
            SkillsCommand::Tag(_) | SkillsCommand::Sync { .. } |
            SkillsCommand::Enable { .. } | SkillsCommand::Disable { .. } => true,
            SkillsCommand::List { tags, preset, untagged, no_preset, .. } =>
                !tags.is_empty() || preset.is_some() || *untagged || *no_preset,
            SkillsCommand::Install { sync, sync_preset, .. } => *sync || sync_preset.is_some(),
            _ => false,
        },
        _ => false,
    };
    if hidden {
        return Err(map_app_err(AppError::invalid_input(
            "This command accesses hidden Foundation state. Use skills install, deploy, undeploy and status.",
        )));
    }
    Ok(())
}

fn foundation_output(value: &mut serde_json::Value) {
    match value {
        serde_json::Value::Object(map) => {
            for key in ["tags", "preset_ids", "presets", "preset_id", "preset_count", "active_preset", "active_preset_id", "active_scenario", "active_scenario_id"] { map.remove(key); }
            if map.contains_key("source_type") { map.remove("enabled"); }
            for item in map.values_mut() { foundation_output(item); }
        }
        serde_json::Value::Array(items) => for item in items { foundation_output(item); },
        _ => (),
    }
}

fn run(cli: Cli) -> anyhow::Result<()> {
    validate_foundation_surface(&cli.command)?;
...
fn run_skills(args: SkillsArgs, store: &SkillStore, json: bool) -> anyhow::Result<()> {
    match args.command {
        ...
        SkillsCommand::Update { reference, all } => {
            ...
            if reports.iter().any(|report| report.error.is_some() || !report.held_back_removals.is_empty()) {
                return Err(anyhow::Error::new(UpdateBatchFailure { reports }));
            }
            print_json(&serde_json::json!({"ok":true,"results":reports}), json);
        }
        SkillsCommand::Check {
            reference,
            all,
...
fn list_skills(store: &SkillStore) -> anyhow::Result<Vec<SkillSummary>> {
    ...
    for skill in store.get_all_skills()? {
        ...
        let mut deployed_to: Vec<String> = targets
            .iter()
            .filter(|target| target.skill_id == skill.id && app_lib::core::foundation_write::target_current(&skill, target))
...
fn skill_status(store: &SkillStore, reference: &str) -> anyhow::Result<SkillStatusReport> {
    ...
    let mut agents: Vec<SkillAgentStatus> = tool_service::list_tool_info(store)
        ...
        .map(|agent| {
            ...
            SkillAgentStatus {
                ...
                deployed: target.is_some_and(|target| app_lib::core::foundation_write::target_current(&skill, target)),
    ...
    for target in unregistered_targets {
        agents.push(SkillAgentStatus {
            ...
            deployed: app_lib::core::foundation_write::target_current(&skill, target),
...
fn run_skill_deployment(
    ...
    if !dry_run {
        app_lib::core::foundation_write::deploy(store, &skill_ids, &agent_keys, deploy).map_err(map_app_err)?;
        let verification =
        ...
        for agent_key in agent_keys {
            ...
            if deployed {
                match current.filter(|target| target.status == "ok") {
                    Some(target) => {
                        if !store.get_skill_by_id(skill_id)?.is_some_and(|skill| app_lib::core::foundation_write::target_current(&skill, target)) {
                            failures.push(format!(
                                "{skill_id}@{agent_key}: target is missing or content differs"
                            ));
                        } else {
...
fn install_local_action(
    ...
    let (skill_id, install_name, central_path) = cmd::install_directory_unlocked(store, &path, name, None, &metadata).map_err(map_app_err)?;
...
fn install_git_action(
    ...
    let result = (|| -> anyhow::Result<(String, String, String)> {
        ...
        let (skill_id, install_name, central_path) = cmd::install_directory_unlocked(store, &skill_dir, name, None, &metadata).map_err(map_app_err)?;
...
fn install_skillssh_action(
    ...
    let result = (|| -> anyhow::Result<(String, String, String)> {
        ...
        let (skill_id, install_name, central_path) = cmd::install_directory_unlocked(store, &skill_dir, Some(&install_name), Some(&destination), &metadata).map_err(map_app_err)?;
        Ok((skill_id, install_name, central_path))
    })();
    git_fetcher::cleanup_temp(&temp_dir);
    let (skill_id, install_name, central_path) = result?;
...
fn run_update(
    ...
    for skill in targets {
        let report = match skill.source_type.as_str() {
            ...
            "builtin" => UpdateReport {
                skill_id: skill.id.clone(), name: skill.name.clone(), source_type: skill.source_type.clone(),
                refreshed: false, error: None, held_back_removals: Vec::new(),
            },
            other => UpdateReport {
                skill_id: skill.id.clone(),
                name: skill.name.clone(),
...
fn run_adopt(
    ...
    for c in &candidates {
        ...
        let (skill_id, install_name, central_path) = cmd::install_directory_unlocked(store, &dir, None, None, &metadata).map_err(map_app_err)?;
...
#[derive(Debug)]
struct UpdateBatchFailure { reports: Vec<UpdateReport> }
impl std::fmt::Display for UpdateBatchFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { f.write_str("Update incomplete; inspect results and review any removals in the App") }
}
impl std::error::Error for UpdateBatchFailure {}

fn error_envelope(err: &anyhow::Error) -> serde_json::Value {
    if let Some(batch) = err.downcast_ref::<UpdateBatchFailure>() {
        return serde_json::json!({"ok":false,"code": if batch.reports.iter().any(|r| r.error.is_some()) { "UPDATE_FAILED" } else { "REMOVAL_REVIEW_REQUIRED" }, "message":batch.to_string(), "results":batch.reports});
    }
...
mod tests {
    #[test]
    fn foundation_rejects_hidden_state_and_scrubs_nested_outputs() {
        for args in [vec!["presets", "list"], vec!["skills", "list", "--tag", "secret"],
            vec!["skills", "install", "./demo", "--sync"], vec!["skills", "sync"]] {
            let cli = Cli::try_parse_from(std::iter::once("skills-manager-cli").chain(args)).unwrap();
            assert!(validate_foundation_surface(&cli.command).is_err());
        }
        let cli = Cli::try_parse_from(["skills-manager-cli", "skills", "deploy", "demo", "--agent", "codex"]).unwrap();
        assert!(validate_foundation_surface(&cli.command).is_ok());
        let mut value = serde_json::json!({"skill":{"source_type":"local","enabled":true,"tags":["secret"],"preset_ids":["hidden"]}, "agents":[{"enabled":true}]});
        foundation_output(&mut value);
        assert_eq!(value, serde_json::json!({"skill":{"source_type":"local"}, "agents":[{"enabled":true}]}));
    }
    ...
    fn skill_and_preset_deployment_round_trip() {
        ...
        central_repo::set_runtime_base_dir_override(Some(tmp.path().to_path_buf()));
        struct Reset;
        impl Drop for Reset { fn drop(&mut self) { central_repo::set_runtime_base_dir_override(None); } }
        let _reset = Reset;
