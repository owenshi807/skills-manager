...
pub const CONTRACT_VERSION: &str = concat!(env!("FOUNDATION_APP_VERSION"), "-foundation-agent-v1-cli", env!("CARGO_PKG_VERSION"));
...
/// agent can drive it.
pub fn ensure_bridge(_app_version: &str) {
    match ensure_bridge_inner(CONTRACT_VERSION) {
        Ok(path) => log::info!("cli bridge: ready at {}", path.display()),
        Err(e) => log::warn!("cli bridge: not available: {e:#}"),
    }
}

pub fn ensure_bridge_inner(app_version: &str) -> Result<PathBuf> {
    ...
    if read_stamp().as_deref() == Some(app_version) && bridge_path().is_file() {
        verify(&bridge_path(), app_version)?;
