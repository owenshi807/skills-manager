...
export function createWorld() {
  ...
  const pondX = 5, pondZ = -4, pondRadius = 3.65, waterY = 0.067;
  const waterPositions = [], waterColors = [], color = new THREE.Color();
  const waterDeep = new THREE.Color('#35AFAB'), waterShallow = new THREE.Color('#4DBEB5');
  const sectors = 36, radialSteps = 6;
  const addWaterTriangle = (a, b, c) => {
    waterPositions.push(...a, ...b, ...c);
    // Shared positions get the same color: the flat pond has a quiet continuous
    // depth gradient instead of independently colored triangular wedges.
    for (const point of [a, b, c]) {
      const radius = Math.hypot(point[0] - pondX, point[2] - pondZ) / pondRadius;
      color.lerpColors(waterDeep, waterShallow, radius);
      waterColors.push(color.r, color.g, color.b);
    }
  };
  const polar = (r, a) => [pondX + Math.cos(a) * r, waterY, pondZ + Math.sin(a) * r];
  for (let ring = 0; ring < radialSteps; ring++) {
    for (let s = 0; s < sectors; s++) {
      ...
      addWaterTriangle(polar(r1, a1), polar(r2, a2), polar(r2, a1));
      if (ring > 0) addWaterTriangle(polar(r1, a1), polar(r1, a2), polar(r2, a2));
    }
  }
  ...
  // The closed basin cap is a real floor below the opaque water plane. Keeping
  // both caps at waterY causes view-dependent fan-shaped depth collisions.
  const basinHeight = 0.15, basinTop = waterY - 0.03;
  mesh(new THREE.CylinderGeometry(3.87, 3.96, basinHeight, 36), m.tealDark, pond, [pondX, basinTop - basinHeight / 2, pondZ], [1, 1, 1], false).name = 'pond-basin';
  ...
  const rippleMaterial = new THREE.MeshBasicMaterial({ color: '#B5E9CD', transparent: true, opacity: 0.28, depthWrite: false, depthTest: true, side: THREE.FrontSide });
  [[6.65, -5.05, 0.58], [5.1, -7.15, 0.36], [3.3, -3.7, 0.28]].forEach(([x, z, radius], i) => {
    const ripple = mesh(new THREE.RingGeometry(radius, radius + 0.018, 24, 1, 0.1, Math.PI * 1.55), rippleMaterial.clone(), pond, [x, waterY + 0.016 + i * 0.002, z], [1, 1, 1], false);
    ripple.name = `pond-ripple-${i + 1}`;
  ...
  mesh(new THREE.CylinderGeometry(0.9, 0.94, 0.22, 12), m.ivory, pedestal, [0, 0.54, 0]).name = 'pedestal-crown';
  // Each inset attaches inside the supporting solid, but its top is raised.
  // Three coincident top caps at .65 previously fought for the same pixels.
  mesh(new THREE.CylinderGeometry(0.59, 0.59, 0.024, 24), m.ivoryDark, pedestal, [0, 0.658, 0], [1, 1, 1], false).name = 'pedestal-raised-ring';
  mesh(new THREE.CylinderGeometry(0.53, 0.53, 0.024, 24), m.ivory, pedestal, [0, 0.68, 0], [1, 1, 1], false).name = 'pedestal-center-inlay';
