...
export function AgentControlSetupCard() {
  ...
  const alreadyInstalled = useMemo(
    () => managedSkills.some((skill) => skill.name === SKILL_NAME && skill.source_type === "builtin" && skill.source_revision === contractVersion),
    [managedSkills, contractVersion]
  );
  ...
  useEffect(() => {
    void api
      ...
      .catch(() => null)
      .then((flag) => setDismissed(flag === "dismissed"));
    void api.getAgentControlVersion().then(setContractVersion).catch(() => setContractVersion(null));
  }, []);

  if (loading || contractVersion === null || dismissed === null || dismissed || alreadyInstalled) return null;
  ...
  const enable = async () => {
    ...
    setBusy(true);
    try {
      await api.authorizeAgentControl(selected);
